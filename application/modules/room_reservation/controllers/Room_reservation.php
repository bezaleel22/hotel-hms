...
class Room_reservation extends MX_Controller
{
	...
	public function index($id = null)
	{
		...
		$data["paymentdetails"] = $this->roomreservation_model->paymentdetails();
		...
		error_log('DEBUG_LOG: Room reservation page loaded');
		error_log('DEBUG_LOG: User accessing room reservation list');
		error_log('DEBUG_LOG: Testing error log message');
	...
	public function submitcheckout($bookedid)
	{
		...
		try {
			...
			echo '<h5>Failed</h5>Error: ' . $e->getMessage();
			return;
		}
	}
	...
	public function checknewroom()
	{
		...
		$totalroomfound = $this->db->select("count(roomid) as totalroom")->from('tbl_roomnofloorassign')->where('roomid', $room_type)->where('status', 1)->get()->row();
		$roomdetails = $this->db->select("*")->from('roomdetails')->where('roomid', $room_type)->get()->row();
		$numberlist = $this->db->select("*")->from('tbl_roomnofloorassign')->where('roomid', $room_type)->where('status', 1)->get()->result();
		$roomlist = '';
	...
	public function newBooking()
	{
		...
		if (empty($this->input->post('bookingid', TRUE))) {
			...
			$grandtotal = ($totalprice) - $promocode;
			...
			$updateData = array(
				...
				'roomrate'                => $rent,
				'offer_discount'          => trim($discount_price, ","),
				'total_price'             => $totalprice - $promocode,
				'paid_amount'             => $advanceamount,
				'coments'                 => 'Booking from admin',
				'checkindate'             => $datefilter1,
				'checkoutdate'            => $datefilter2,
